...
export async function POST() {
  ...
  const acioandosIds = new Set<string>();
  if (admin) {
    ...
    if (Array.isArray(data)) {
      for (const row of data) {
        if (row.lead_id) acioandosIds.add(String(row.lead_id));
      }
    }
  }
  ...
  const naoAcionados = allLeads.filter((lead) => {
    ...
    const hasCalls = acioandosIds.has(lead.id);
  ...
  // 5. Mesclar de volta na coleção completa e persistir
