...
function renderFunnelBlock(params: {
  ...
  return (
    <section className="rounded-2xl border border-slate-700/70 bg-[#111A2E]/82 p-4 shadow-[inset_0_0_0_1px_rgba(255,255,255,0.03),0_16px_34px_rgba(2,6,23,0.35)]">
        ...
        {steps.map((step, index) => {
          ...
          return (
            <div key={step.id} className={index === 0 ? "flex justify-center" : "-mt-2 flex justify-center"}>
              <div
                className="relative h-[78px] max-w-[620px] overflow-hidden border border-white/10"
                style={{
              ...
              >
                ...
                <div className="relative flex h-full items-center justify-between px-4">
                  ...
                  <div className="flex items-center gap-2 rounded-full border border-white/20 bg-black/20 px-3 py-1.5 text-white shadow-[0_8px_18px_rgba(2,6,23,0.3)]">
                    {iconForStep(step.iconId)}
                    <span className="text-2xl font-semibold tracking-[-0.03em] md:text-[30px]">{displayValue}</span>
                  </div>
                </div>
              </div>
            </div>
...
export function DashboardFunnel({ ligacoes, atendidas, decisor, agendamentos }: DashboardFunnelProps) {
  ...
  const percentSteps: FunnelStep[] = useMemo(() => {
    const safeLigacoes = Math.max(0, ligacoes);
    const conversionPercent = (value: number) => {
      if (safeLigacoes <= 0) return 0;
      return (Math.max(0, value) / safeLigacoes) * 100;
    };
    ...
    return [
      {
        ...
        iconId: "atendidas",
        label: "Atendidas",
        value: conversionPercent(atendidas),
        gradientFrom: "#8B5CF6",
        gradientTo: "#6D28D9",
      },
      {
        ...
        iconId: "decisor",
        label: "Decisor",
        value: conversionPercent(decisor),
        gradientFrom: "#22C55E",
        gradientTo: "#15803D",
      },
      {
        ...
        iconId: "agendamentos",
        label: "Agendamentos",
        value: conversionPercent(agendamentos),
        gradientFrom: "#F97316",
        gradientTo: "#DC2626",
      },
    ];
    ...
    <section className="relative overflow-hidden rounded-3xl border border-slate-800/80 bg-[#0F172A]/95 p-5 shadow-[inset_0_0_0_1px_rgba(255,255,255,0.03),0_28px_68px_rgba(2,6,23,0.45)] md:p-6">
      ...
      <div className="relative space-y-4">
        {renderFunnelBlock({
          ...
          subtitle: "Dados reais consolidados do modulo de ligacoes: Ligacoes -> Atendidas -> Decisor -> Agendamentos.",
          steps: absoluteSteps,
          displayMode: "absolute",
          widthOptions: {
            maxReferenceValue: Math.max(1, ...absoluteSteps.map((step) => step.value)),
            minWidthPercent: 34,
            firstMinWidthPercent: 86,
            shrinkPerStep: 10,
          },
        })}
        ...
        {renderFunnelBlock({
          ...
          subtitle:
            "Atendidas = atendidas/ligacoes, Decisor = decisor/ligacoes, Agendamentos = agendamentos/ligacoes.",
          steps: percentSteps,
          displayMode: "percent",
          widthOptions: {
            maxReferenceValue: 100,
            minWidthPercent: 26,
            firstMinWidthPercent: 58,
            shrinkPerStep: 8,
          },
        })}
      </div>
